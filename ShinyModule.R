  ...
  tagList(
    ...
    sidebarLayout(
      sidebarPanel(
        ...
        numericInput(ns("light_threshold"), 
                     h4("Light threshold"), 
                     value = 5.5,
                     step = 0.1),
        br(),
        ...
        numericInput(ns("problem_threshold"), 
      ...
      mainPanel(
        ...
        withSpinner(plotOutput(ns("plotall"),
                               height = "150px")),
        ...
        numericInput(ns("time_window"), "Editing window length",
...
shinyModule <- function(input, output, session, data) {
  ...
  overlap_window <- reactive ({
    if (input$edit_units == "days")
    {overlap_size_in_sec <- input$overlap_window * 24 * 60 * 60 #This converts days to seconds
    return(overlap_size_in_sec)}
    else
    {{overlap_size_in_sec <- input$overlap_window * 60 * 60     #This converts hours to seconds
    return(overlap_size_in_sec)}}   
  })      
  ...
  edited_twilights <- reactive ({
    ...
    edited_twilights$type[edited_twilights$Rise==TRUE] <- 1
